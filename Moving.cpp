...
struct Local {
  ...
  {
    accessor.setValue(iter.getCoord(), iter.getCoord().asVec3s());
  }

  static inline void mul(const openvdb::Vec3SGrid::ValueOnIter& iter)
  {
    iter.setValue(*iter * 10);
  }
};
...
int main()
{
    ...
    openvdb::FloatGrid::Ptr grid =
        openvdb::tools::createLevelSetSphere<openvdb::FloatGrid>(
            /*radius=*/radius, /*center=*/openvdb::Vec3f(0, 0, 0),
...
// Randomize the point positions.
    ...
    std::cout << "Simulating for 3 seconds" << std::endl;
    
    int timeSteps = 50;
    float t = 0.0;
    float dt = 0.0003;

    openvdb::BoolGrid::ConstPtr surfaceMask =
      openvdb::tools::interiorMask(*grid);
    openvdb::Vec3SGrid::ConstPtr surfaceVelocity = openvdb::Vec3SGrid::create();
      //openvdb::tools::foreach(surfaceVelocity->beginValueAll(), Local::op);
      //openvdb::Vec3SGrid::create(-windVelocity);
    openvdb::FloatGrid::Ptr objectGrid = grid->deepCopy();
    openvdb::tools::erodeVoxels(objectGrid->tree(), 1);
    openvdb::Vec3SGrid::Ptr surfaceMovement =
      openvdb::tools::gradient(*objectGrid);
    openvdb::tools::foreach(surfaceMovement->beginValueOn(), Local::mul);
    surfaceMovement->setTransform(flowVel->transform().copy());
    openvdb::tools::compSum(*flowVel, *surfaceMovement);
    openvdb::math::Transform &objectXForm = flowVel->transform();
    for (; t < 3.0; ) {
	...
	objectXForm.postTranslate(-dt*timeSteps*windVelocity);
	openvdb::points::advectPoints(*pointGrid, *flowVel, 4, dt, timeSteps);
